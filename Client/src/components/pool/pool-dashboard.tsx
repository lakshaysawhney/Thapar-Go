...
import { transportModes, getUniqueStartPoints, getUniqueEndPoints } from "@/data/pool-data"
...
export default function PoolDashboard() {
  ...
  const { filters, updateFilter, resetFilters, filteredPools, fareRange } = usePoolFilters()
      ...
      <div className="container mx-auto p-4">
        ...
        <Tabs defaultValue="all" className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredPools.length > 0 ? (
                filteredPools.map((pool) => (
                  <PoolCard key={pool.id} pool={pool} onClick={() => handlePoolSelect(pool)} />
                ))
              ) : (
          ...
          <TabsContent value="my">
            <div className="text-center py-10 text-muted-foreground">You haven't created any pools yet</div>
          </TabsContent>
        </Tabs>
      </div>

      {/* Pool Details Dialog */}
      <PoolDetails pool={selectedPool} open={!!selectedPool} onOpenChange={(open) => !open && setSelectedPool(null)} />

      {/* Create Pool Dialog */}
      <Dialog open={isCreatePoolOpen} onOpenChange={setIsCreatePoolOpen}>
        <DialogContent className="sm:max-w-[600px] bg-background/95 backdrop-blur-sm max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl text-primary">Create New Pool</DialogTitle>
            <DialogDescription>Fill in the details to create a new car pool.</DialogDescription>
          </DialogHeader>
